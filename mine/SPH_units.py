...
class model:
    ...
    def __init__(self,**kwargs):
        ...
        values  =   dict(M=10e5, N=500, kernel='gaussian', initial_distribution='uniform', n_neighbors=7, initial_dispersion=0, initial_omega=0, initial_radius=50, initial_temp=5)
        values  =   aux.update_values(values,kwargs)

        # add initial attributes
        for key in values: setattr(self,key,values[key])
        self.m  =   self.M / self.N
        G_model /=  self.initial_radius**3
    ...
    def __gravity(self):
        ...
        return A * G_model * self.m**2 / self.initial_radius**2
